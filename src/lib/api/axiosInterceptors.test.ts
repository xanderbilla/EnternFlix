...
describe("axios interceptors", () => {
  ...
  describe("response interceptor", () => {
    it("passes through 2xx responses unchanged", async () => {
      ...
      const fulfilled = (instance.interceptors.response as any).handlers[0]
        .fulfilled;
    ...
    it("logs an error for 5xx server errors", async () => {
      ...
      const rejected = (instance.interceptors.response as any).handlers[0]
        .rejected;
  ...
  describe("request interceptor", () => {
    it("passes config through unchanged on success", () => {
      ...
      const fulfilled = (instance.interceptors.request as any).handlers[0]
        .fulfilled;

      const cfg = { url: "/x", headers: {} } as any;
    ...
    it("logs and rethrows on request error", async () => {
      const instance = axios.create();
      configureRequestInterceptor(instance);
      const rejected = (instance.interceptors.request as any).handlers[0]
        .rejected;
